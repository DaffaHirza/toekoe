                ...
                <div class="flex flex-col">
                    <div class="flex-1">
                        ...
                        <div class="flex items-center gap-2 mb-4">
                            <span class="text-xs px-3 py-1 rounded-full {{ $product->kondisi == 'baru' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700' }}">
                        ...
                        <div class="mb-6">
                            <p class="text-4xl font-bold text-blue-600">Rp {{ number_format($product->harga, 0, ',', '.') }}</p>
                        </div>
                        ...
                        <div class="bg-gray-50 rounded-lg p-4 mb-6">
                            ...
                            <p class="text-sm text-gray-600"><strong>Alamat:</strong> {{ $product->user->alamat }}, RT {{ $product->user->rt }}/RW {{ $product->user->rw }}, {{ $product->user->nama_kelurahan }}, {{ $product->user->kabupaten_kota }}, {{ $product->user->provinsi }}</p>
                        </div>
                    </div>

                    <div class="flex gap-3">
                        <button class="flex-1 bg-pink-500 hover:bg-pink-600 text-white font-semibold py-3 rounded-lg transition duration-300">
                            <i class="bi bi-heart"></i> Wishlist
                        </button>
                        <button class="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 rounded-lg transition duration-300">
                            <i class="bi bi-cart"></i> Add to Cart
                        </button>
                    </div>

                    <a href="{{ route('home') }}" class="mt-4 text-center text-blue-600 hover:text-blue-700 font-medium">
                        ← Kembali ke Beranda
                    </a>
                </div>
            </div>
            ...
            <div class="mt-8 bg-white rounded-xl shadow-lg p-8">
                    ...
                    <div class="flex items-center gap-4">
                        <div class="text-center">
                            ...
                            <div class="flex justify-center mt-2">
                                @for($i = 1; $i <= 5; $i++)
                                    <svg class="w-5 h-5 {{ $i <= ($product->reviews->avg('rating') ?? 0) ? 'fill-yellow-400' : 'fill-gray-300' }}" viewBox="0 0 24 24">
                ...
                <div class="border-t pt-6">
                    ...
                    <form action="{{ route('produk.review.store', $product->id) }}" method="POST" class="space-y-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium mb-2">Komentar <span class="text-red-500">*</span></label>
                        ...
                        <div class="border-b last:border-b-0 pb-4 mb-4">
                            <div class="flex items-start gap-4">
                                ...
                                <div class="flex-1">
                                    ...
                                    <div class="flex mb-2">
                                        @for($i = 1; $i <= 5; $i++)
                                            <svg class="w-4 h-4 {{ $i <= $review->rating ? 'fill-yellow-400' : 'fill-gray-300' }}" viewBox="0 0 24 24">
                                                <path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/>
                                            </svg>
                                        @endfor
                                    </div>
