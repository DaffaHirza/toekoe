...
@section('content')
    ...
    <div x-data="{
        gambarPreview: '{{ $product->image ? asset('storage/' . $product->image) : null }}',
        hargaInput: '{{ old('harga', $product->harga) }}',
        kondisi: '{{ old('kondisi', $product->kondisi) }}'
        ...
        <form action="{{ route('seller.pages.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="deskripsi" rows="4" class="w-full px-4 py-2 rounded-xl border dark:border-slate-700 bg-slate-50"
                    placeholder="Jelaskan produk secara detail..." required>{{ old('deskripsi', $product->deskripsi) }}</textarea>
                @error('deskripsi')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
                ...
                <div class="flex gap-4">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="kondisi" value="baru" x-model="kondisi"
                            {{ old('kondisi', $product->kondisi) == 'baru' ? 'checked' : '' }} class="w-4 h-4 text-indigo-600">
                        <span class="text-sm ">🆕 Baru</span>
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="kondisi" value="bekas" x-model="kondisi"
                            {{ old('kondisi', $product->kondisi) == 'bekas' ? 'checked' : '' }} class="w-4 h-4 text-indigo-600">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input x-model="hargaInput" type="number" name="harga" value="{{ old('harga', $product->harga) }}"
                        class="w-full px-4 py-2 rounded-xl border dark:border-slate-700 bg-slate-50"
            ...
            <div class="space-y-2">
                ...
                @if($product->image)
                    <div class="mb-3">
                        ...
                        <img src="{{ asset('storage/' . $product->image) }}" 
                            class="w-48 h-48 object-cover rounded-xl shadow-md border-2 border-slate-200">
                    </div>
                @endif
                ...
                <input type="file" name="image" accept="image/*"
                    ...
                    class="w-full px-4 py-2 rounded-xl border dark:border-slate-700 bg-slate-50">
                <p class="text-xs text-slate-500">Format: JPG, PNG, GIF (Max: 2MB) - Kosongkan jika tidak ingin mengubah gambar</p>

                @error('image')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror

                <template x-if="gambarPreview && gambarPreview !== '{{ $product->image ? asset('storage/' . $product->image) : '' }}'">
                    <div class="mt-3">
                        <p class="text-xs text-slate-500 mb-2">Preview gambar baru:</p>
